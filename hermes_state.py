...
class SessionDB:
    ...
    def resolve_resume_session_id(self, session_id: str) -> str:
        ...
        with self._lock:
            ...
            # Walk descendants via a single recursive CTE.
            # At each step, pick the most-recently-started child session;
            # stop once we find one with messages. Max depth 32.
            q = """
            WITH RECURSIVE descendant_path(id, depth) AS (
                SELECT id, 1
                FROM (SELECT id FROM sessions WHERE parent_session_id = ? ORDER BY started_at DESC, id DESC LIMIT 1)
                ...
                SELECT (
                    SELECT id
                    FROM sessions
                    WHERE parent_session_id = dp.id
                    ORDER BY started_at DESC, id DESC
                    LIMIT 1
                ), depth + 1
                FROM descendant_path dp
                WHERE dp.id IS NOT NULL AND dp.depth < 32
            )
            SELECT dp.id
            FROM descendant_path dp
            WHERE dp.id IS NOT NULL
              AND EXISTS (SELECT 1 FROM messages WHERE session_id = dp.id LIMIT 1)
            LIMIT 1;
            """
            try:
                row = self._conn.execute(q, (session_id,)).fetchone()
                if row is not None:
                    res_id = row["id"] if hasattr(row, "keys") else row[0]
                    if res_id:
                        return res_id
            except Exception:
                return session_id
