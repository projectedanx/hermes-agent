...
**Important:** Always use `load_godmode.py` instead of loading individual scripts directly. The individual scripts have `argparse` CLI entry points and `__name__` guards that could break if loaded dynamically. The loader handles this.
...
9. **Always use `load_godmode.py` in execute_code** — The individual scripts (`parseltongue.py`, `godmode_race.py`, `auto_jailbreak.py`) have argparse CLI entry points with `if __name__ == '__main__'` blocks. When imported or run, argparse could fire unexpectedly. The `load_godmode.py` loader handles this by setting sys.argv to avoid conflicts.
