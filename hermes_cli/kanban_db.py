    ...
    if runs_exist:
        with write_txn(conn):
            ...
            if inserted:
                ...
                # CAS: Clean up orphaned runs where the task's current_run_id wasn't updated
                # (e.g. if another process already set it, which shouldn't happen under
                # write_txn, but belt-and-suspenders).
                run_ids = [r[0] for r in inserted]
                chunk_size = 500
                for i in range(0, len(run_ids), chunk_size):
                    ...
                    conn.execute(
                        ...
                        AND id NOT IN (
                            SELECT current_run_id FROM tasks WHERE current_run_id IN ({placeholders}) AND current_run_id IS NOT NULL
                        )
                        """,
                        [now] + chunk + chunk,
                    )
